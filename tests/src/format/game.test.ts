...
describe('When using Game class as an parser then', () => {
    it(
        'fromCsvString return a game with n = 3 players and (2 ^ n) - 1 = 7 coalitions',
        async () => {
            ...
            expect(Object.keys(game.payoffs)).toHaveLength(
                Object.keys(THREE_PLAYERS_GAME_STRUCTURE_MOCK.payoffs).length
            );
        },
        fromCsvTestTimeout
    );
    ...
    it(
        'fromCsvString assigns proper coalition values',
        async () => {
            const game = await new Game().fromCsvString(THREE_PLAYERS_GAME_CSV_MOCK);
    ...
    it(
        'fromCsvString should rise error when an empty string is given',
        async () => {
            expect.assertions(2);
    ...
    it(
        'fromCsvString should rise error when a string with no value rows is given',
        async () => {
            expect.assertions(2);
    ...
    it(
        'fromCsvString should rise error when a row with invalid field length is given',
        async () => {
            expect.assertions(2);
    ...
    it(
        'fromCsvString should rise error when a bad column delimiter is used',
        async () => {
            expect.assertions(3);
    ...
    it(
        'fromCsvString should rise error when payoffs are of type different from float',
        async () => {
            expect.assertions(3);
    ...
    it(
        'fromCsvString should rise error when some coalition has no players',
        async () => {
            expect.assertions(3);
